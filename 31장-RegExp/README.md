...
- 순서와 상관없이 하나 이상의 플래그를 동시에 설정할 수 있따.
...
위 예제는 패턴을 or로 한 번 이상 반복하는 것인데 이를 간단하게 표현하면 다음과 같다. `[ ]` 내의 문자는 or로 동작하고 뒤에 +를 사용하면 앞선 패턴을 한 번 이상 반복한다.
...
`\w`는 `알파벳`, `숫자`, `언더스코어`를 의미하며 `[A-Za-z0-9]`와 같다.
`\W`는 `\w`와 반대로 동작하며 `알파벳`, `숫자`, `언더스코어` 아닌 `문자`를 의미한다.
...
- `?`은 앞선 패턴(s)이 최대 한 번(0반 포함) 이상 반복되는지를 의미한다.
